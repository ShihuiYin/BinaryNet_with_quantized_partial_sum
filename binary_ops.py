...
def TernarySignNumpy(x):
    return np.float32(np.clip(np.round(x), -1, 1))
    
def TernarySignTheano(x):
    return T.cast(T.clip(T.round(x), -1, 1), theano.config.floatX)

def TernarySignTheano_v2(x):
    return T.cast(T.clip(T.round(2 * x), -1, 1), theano.config.floatX)

def TernarySignTheano_v3(x):
    return T.cast(T.clip(T.round(0.5 * x), -1, 1), theano.config.floatX)
